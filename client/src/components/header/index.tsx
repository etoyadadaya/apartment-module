...
const Header: FC<IHeaderProps> = ({callback, setIsActive}) => {
    ...
    const mainHandleChange = useCallback((e: FormEvent<HTMLSelectElement>) => {
        callback(parseSort(e.currentTarget.value));
        additionalRef.current.value = "id";
    }, [callback]);

    const additionalHandleChange = useCallback((e: FormEvent<HTMLSelectElement>) => {
        callback(parseSort(e.currentTarget.value));
                ...
                <nav className={styles.navbar}>
                    <div className={styles.buttons}>
                        <Button onClick={() => setIsActive(false)}>
                            <svg height="24" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M47 23H23V47H47V23Z" fill="black"/>
                                <path d="M77 23H53V47H77V23Z" fill="black"/>
                                <path d="M47 53H23V77H47V53Z" fill="black"/>
                                <path d="M77 53H53V77H77V53Z" fill="black"/>
                            </svg>
                        </Button>
                        <Button onClick={() => setIsActive(true)}>
                            <svg height="24" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect x="23" y="23" width="24" height="24" fill="black"/>
                                <rect x="53" y="31" width="24" height="8" fill="black"/>
                                <rect x="23" y="53" width="24" height="24" fill="black"/>
                        ...
                        <Button>
                            <select ref={additionalRef} onChange={additionalHandleChange} className={styles.select}>
